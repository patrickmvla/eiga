...
export const ReviewTextSchema = z
  ...
  .refine(
    ...
    { error: `Review must be ${REVIEW_MIN_WORDS}–${REVIEW_MAX_WORDS} words.` }
  )
  .max(8000, "Review is too long")
  .transform((s) => trimCompact(s)); // character guard for safety
...
const lineList = z.union([z.string(), z.array(z.string())]).transform((val) => {
  const arr = typeof val === "string" ? val.split(/\r?\n/) : val;
  return arr.map((s) => s.trim()).filter((s) => s.length > 0);
});

/* YYYY-MM-DD guard (optionally Monday) */
const DateYmdSchema = z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Use YYYY-MM-DD");
...
export const AdminScheduleFilmSchema = z.object({
  ...
  poster_url: z.url().max(500).optional(),
  ...
  themes: lineList
    ...
    .refine((arr) => arr.length >= 1, { error: "Add at least one theme." })
    .refine((arr) => arr.every((s) => s.length <= 120), {
      error: "Themes should be ≤120 characters each.",
    }),
  technical: lineList
    .transform((arr) => arr.slice(0, 10))
    .refine((arr) => arr.every((s) => s.length <= 120), {
      error: "Technical notes should be ≤120 characters each.",
    })
    .optional()
    .default([]),
  context: z
...
export const RatingEditSchema = z
  ...
  .superRefine((val, ctx) => {
    if (val.score == null && val.review == null) {
      ctx.addIssue({
        code: "custom",
        path: ["score"],
