...
export async function POST(req: Request) {
  ...
  try {
    await db.execute(
      sql`update "waitlist" set status = 'archived' where id = ${parsed.data.id}`
    );
  } catch {
    // table/column may not exist; ignore in best-effort mode
  }

  return wantsJSON(req)
    ? toJSON({ ok: true })
    : NextResponse.redirect(abs(req, "/invites?archived=1"), 303);
}