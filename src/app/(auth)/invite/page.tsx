...
const Page = ({ params, searchParams }: PageProps) => {
  ...
  const status = getParam(searchParams, 'status'); // optional: 'invalid' | 'used' | 'expired'
  const error = getParam(searchParams, 'error');   // optional fallback error text
  ...
  return (
    <main className="mx-auto w-full max-w-md px-4 py-10 md:py-14">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs ${
              status === 'invalid'
                ...
                : 'border border-olive-500/30 bg-olive-500/10 text-olive-200'
            }`}
          >
            {status === 'invalid'
              ? 'Invalid code'
      ...
      <Card padding="lg">
        <form
          method="POST"
          action="/api/auth/redeem-invite" /* TODO: implement server route */
          acceptCharset="UTF-8"
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-neutral-500">
              We’ll use this to send a one-time sign-in link to complete setup.
            </p>
          </div>
          ...
          <button
            ...
            disabled={status === 'invalid' || status === 'used' || status === 'expired'}
          >
            Redeem invite
          </button>
