...
// NOTE: The PageProps type is correct as per Next.js docs.
// The issue is how we handle the props inside the component due to a potential bug.
// type PageProps = {
//   params: { code: string };
...
const statusBadge = (status: StatusParam) => {
  if (status === "invalid") {
    return {
      cls: "border border-red-500/30 bg-red-500/10 text-red-300",
      text: "Invalid code",
    };
  }
  if (status === "used") {
    return {
      cls: "border border-yellow-500/30 bg-yellow-500/10 text-yellow-300",
      text: "Already used",
    };
  }
  if (status === "expired") {
    return {
      cls: "border border-orange-500/30 bg-orange-500/10 text-orange-300",
      text: "Expired",
    };
  }
  return {
    cls: "border border-olive-500/30 bg-olive-500/10 text-olive-200",
...
// MODIFICATION: We add 'any' to props to bypass the initial type check,
// then await them inside, which seems to be what Next.js 15 is expecting in your case.
